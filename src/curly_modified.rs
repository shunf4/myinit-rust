//! Implementation for simple format strings using curly braces. (Modified)
//!
...
use dynfmt::{ArgumentResult, ArgumentSpec, Error, Format, Position};
...
///
/// [`SimpleCurlyFormat`]: struct.SimpleCurlyFormat.html
pub struct SimpleCurlyModifiedIter<'f> {
    captures: CaptureMatches<'static, 'f>,
}

impl<'f> SimpleCurlyModifiedIter<'f> {
    fn new(format: &'f str) -> Self {
        SimpleCurlyModifiedIter {
            captures: PYTHON_RE.captures_iter(format),
        }
    }
}

impl<'f> Iterator for SimpleCurlyModifiedIter<'f> {
...
/// ```
pub struct SimpleCurlyModifiedFormat;

impl<'f> Format<'f> for SimpleCurlyModifiedFormat {
    type Iter = SimpleCurlyModifiedIter<'f>;

    fn iter_args(&self, format: &'f str) -> Result<Self::Iter, Error<'f>> {
        Ok(SimpleCurlyModifiedIter::new(format))
    }
}
